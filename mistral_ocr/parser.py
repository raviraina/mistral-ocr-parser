...
class MistralOCRParser:
    ...
    def parse_pdf(self, pdf_path: Union[str, Path], output_path: Optional[Union[str, Path]] = None) -> str:
        """
        ...
        for block in tqdm(blocks, desc="Processing blocks"):
            ...
            if block_type == "text":
                ...
                if image_base64:
                    ...
                    image_description = generate_image_description(image_base64, self.client)
                    
                    # Add image with description to markdown
                    markdown_parts.append(f"![Image]({image_data.get('url', 'image_placeholder.png')})\n")
                    markdown_parts.append(f"*Image Description: {image_description['description']}*\n\n")
...
def parse_pdf(pdf_path: Union[str, Path], output_path: Optional[Union[str, Path]] = None, api_key: Optional[str] = None) -> str:
    """
    Parse a PDF document using Mistral's OCR API and generate structured markdown output.
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Parse PDF documents using Mistral's OCR API")
